...
public class Ingredient {
   ...
   *
   * <p>Slight modifications have been made to make the pattern more amenable to extracting data.
   * Several capturing groups are used for this purpose:
   *
   * <ol>
   *   <li>The fraction string, which can be parsed by {@link FractionUtils#parseFraction(String)}.
   *   <li>The unit.
   *   <li>The item.
   *   <li>All the preparation directions as one big capturing group (which may be an empty string,
   *       if there are none).
   * </ol>
   */
  public static final Pattern INGREDIENT_PATTERN =
      Pattern.compile(
          "([1-9][0-9]* ?(?:(?:/| [1-9][0-9]* ?/) ?[1-9][0-9]*)?) ([^ ]+) ([^,]*[^, ])((:?, [^,]*[^, ])*)");

  /** The quantity of the specified item, in terms of the specified unit. */
  @NonNull private Fraction quantity;

  /** The unit of measurement which the specified quantity uses (e.g. "cup"). */
  @NonNull private String unit = "each";

  /** The item of which this ingredient consists (e.g. "apple"). */
  @NonNull private String item;

  /** How the ingredient should be prepared prior to use (e.g. "chopped"). */
  @NonNull private List<String> preparation = new ArrayList<>();

  /**
   * Parses a human-readable ingredient string.
   *
   * <p>The description format consists of four parts, one of which is optional:
   *
   * <ol>
   ...
   *       should be used.
   *   <li>The item (e.g. flour). The item name must not contain a comma, since this is used to
   *       denote the start of the preparation instructions.
   *   <li>Preparation instructions (optional). These are given as a comma-separated list, beginning
   *       with a comma after the material.
   * </ol>
   *
   ...
   *
   * @return the ingredient, as a human-readable string in the format understood by {@link
   *     #parse(String)}.
   */
  @Override
  public String toString() {
    return FractionUtils.toMixedNumberString(quantity)
        + " "
        + unit
        + " "
        + item
